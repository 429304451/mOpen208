...
aDic.desc = "浮雕";
// 如果没有指明图片 默认使用的双图片对比 
aDic.path = "easy/ShaderRelief"; // 路径 为便于查找
...
void main () {
    ...
    vec4 color;
    color.rgb = vec3(0.5);
    color -= texture2D(texture, uv0 - onePixel) * pos.x;
    color += texture2D(texture, uv0 + onePixel) * pos.y;
...
`
    ...
    initMsg.changeDSp(initMsg.imgPath.frozen, function () {
        ...
        uiShader.changeSld2(0.23);
        this.changePos(cc.v2(201, 108))
    }.bind(this))
}
